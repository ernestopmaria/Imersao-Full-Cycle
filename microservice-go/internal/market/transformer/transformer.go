...
import (
	"github.com/ernestopmaria/imersao13/go/internal/market/dto"
	"github.com/ernestopmaria/imersao13/go/internal/market/entity"
)
...
func TransformOutput(order *entity.Order) *dto.OrderOutput {
	...
	var transactionsOutput []*dto.TransationOutput
	for _, t := range order.Transactions {
		transactionOuput := &dto.TransationOutput{
			TransactionID: t.ID,
		...
		transactionsOutput = append(transactionsOutput, transactionOuput)
	}

	output.TransationOutput = transactionsOutput
	return output
}
